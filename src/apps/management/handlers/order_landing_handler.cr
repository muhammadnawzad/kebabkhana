module Management
  class OrderLandingHandler < Marten::Handler
    ...
    def get
      ...
      user_not_verified = !request.user!.is_phone_verified
      order = Order.filter(user_id: request.user!.id, batch_id: active_batch.first.not_nil!.id).first.not_nil! if has_already_ordered

      render("home/index.html", context: {has_already_ordered: has_already_ordered, pending_payments: pending_payments, no_active_batch: no_active_batch, user_not_verified: user_not_verified, order: order})
    end
  end
end
